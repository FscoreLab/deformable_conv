...
class DeformRoIPooling(nn.Module):
    def __init__(
        self, out_size, out_channels, no_trans, group_size=1, part_size=None, sample_per_part=4, trans_std=0.0,
    ):
        super().__init__()
    ...
    def forward(self, data, rois, offset, spatial_scale):
        if self.no_trans:
...
class DeformRoIPoolingPack(DeformRoIPooling):
    def __init__(
        ...
        super().__init__(out_size, out_channels, no_trans, group_size, part_size, sample_per_part, trans_std)
    ...
    def forward(self, data, rois, spatial_scale):
        ...
        if self.no_trans:
            ...
            roi_feats = deform_roi_pooling(
                data,
                rois,
                offset,
                spatial_scale,
                (self.out_size[0] * 2, self.out_size[1] * 2),
                self.out_channels,
                self.no_trans,
                self.group_size,
                self.part_size * 2,
